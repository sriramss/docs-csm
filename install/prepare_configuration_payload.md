...
and an extra configuration file which that installation generated. The *first time* install scenario requires using the csi config tool to pass arguments to csi as well as the creation of a number of [Configuration Payload Files}(#configuration-payload-files).
