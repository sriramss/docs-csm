...
and an extra configuration file which that installation generated. The *first time* install scenario requires using the csi config tool to pass arguments to it as well as the creating a number of [Configuration Payload Files}(#configuration-payload-files).
