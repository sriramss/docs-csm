...
1. To execute this command you will need the following:
    ...
    > The hsn_connections.json, ncn_metadata.csv, switch_metadata.csv, and optionally application_node_config.yaml files in the current directory as well as values for the flags listed below.
    ...
    > If you have a `system_config.yaml` file from a pervious configuration payload generated by CSI, then it can be used to supply configuration options to CSI instead of specify CLI flags. The `system_config.yaml` must be in the current directory.
